...
2.1. Отслеживать за счет смещения границ (динамика смещения квантилей разбиения)
2.2. Разбить клиентов на тестовую (рассылка на основе RFM анализа) и контрольную (без рассылки)  группы и сравнить конвертации. (Создать пул клиентов для тестовой и контрольной группы из каждого сегмента  в одинаковой пропорции)
...
3.3. Задание четких границ сегментов (тогда автоматически исключается первый пункт). Анализируем  динамику наполняемости каждой группы по количеству. 
